...
$sth = $dbh->query("SELECT obj_Id FROM `object` WHERE obj_Name='$targetName' AND obj_Type=0", PDO::FETCH_ASSOC);
$rows = $sth->fetchAll();
foreach ($rows as $value) {
	$target=$value['obj_Id'];
}
if (empty($target)) {
	$sth = $dbh->query("SELECT obj_Id FROM `object` WHERE res_Name='$targetName' AND obj_Type=1", PDO::FETCH_ASSOC);
	$rows = $sth->fetchAll();
	foreach ($rows as $value) {
		$target=$value['obj_Id'];
	}
